...
echo $1
JOB_NAME=$1
echo output_label=${1:-JOB_NAME}
output_label=${1:-JOB_NAME}
echo output_label=${scm-server:-scm-server}
output_label=${scm-server:-scm-server}


echo JOB_DIR=/var/vcap/jobs/$JOB_NAME
export JOB_DIR=/var/vcap/jobs/$JOB_NAME
echo chmod 755 $JOB_DIR
...
echo 'source $JOB_DIR/helpers/ctl_utils.sh'
source $JOB_DIR/helpers/ctl_utils.sh
echo 'redirect_output ${output_label}'
redirect_output ${output_label}


echo 'export HOME=${HOME:-/home/vcap}'
export HOME=${HOME:-/home/vcap}

# Add all packages' /bin & /sbin into $PATH
for package_bin_dir in $(ls -d /var/vcap/packages/*/*bin)
do
  echo PATH=${package_bin_dir}:$PATH
  export PATH=${package_bin_dir}:$PATH
done

echo export LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-''} # default to empty
export LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-''} # default to empty
for package_bin_dir in $(ls -d /var/vcap/packages/*/lib)
do
  echo echo export LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-''} # default to empty
  export LD_LIBRARY_PATH=${package_bin_dir}:$LD_LIBRARY_PATH
done

# Setup log, run and tmp folders
echo export RUN_DIR=/var/vcap/sys/run/$JOB_NAME
export RUN_DIR=/var/vcap/sys/run/$JOB_NAME
echo export LOG_DIR=/var/vcap/sys/log/$JOB_NAME
export LOG_DIR=/var/vcap/sys/log/$JOB_NAME
echo export TMP_DIR=/var/vcap/sys/tmp/$JOB_NAME
